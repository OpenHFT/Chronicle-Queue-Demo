...
An event-driven account management system utilising Chronicle libraries for high-performance and low-latency operations.
This project demonstrates a reference architecture for creating accounts, performing transfers, and capturing checkpoints of system state, supported by comprehensive testing and benchmarking tools.
...
A typical flow:
. A request (e.g., `createAccount`) arrives via a Chronicle channel.
. `AccountManagerImpl` receives the request, delegates validation and business logic to `AccountService`.
. `AccountService` returns success or a failure reason.
. `AccountManagerImpl` emits corresponding events (e.g., `onCreateAccount` or `createAccountFailed`).
