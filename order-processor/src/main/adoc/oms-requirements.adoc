...
This approach provides _traceability_, as all inbound and outbound events are persisted.

== Purpose and Scope

The primary purposes of `OMSImpl` are:

. *Receive and Validate* incoming messages (via the framework, which calls `validate()` on DTOs prior to invocation).
. *Process* inbound messages (e.g., new orders, cancellation requests).
. *Generate* outbound messages (e.g., execution reports, cancellation rejects).
. *Publish* outbound events to a Chronicle Queue, after validation via `validate()`.
...
. `newOrderSingle(NewOrderSingle nos)`
. `cancelOrderRequest(CancelOrderRequest cor)`
...
* If the inbound `NewOrderSingle` fails validation (missing fields, invalid values), the system _must_ create and send an `OrderCancelReject` instead of an `ExecutionReport`.
...
. *Order Creation*: Creates and populates an `ExecutionReport` from a `NewOrderSingle`.
. *Order Rejection*: Returns an `OrderCancelReject` for both single-order and mass-cancel requests, or if validation fails.
. *DTO Mapping*: Maps inbound DTO fields to outbound DTO fields (`sender ↔ target`, `symbol`, `clOrdID`, etc.).
. *Time/ID Generation*: Uses `SystemTimeProvider.CLOCK.currentTimeNanos()` to assign `orderID`.
. *Validation Flow*: Relies on the framework to call `validate()` on input and output DTOs. If invalid, the framework or `OMSImpl` triggers appropriate rejection/error handling.
...
. *Clone the source code* for the Order Processor Demo (the project containing `pom.xml` and these source files).
. *Build the project*:
+
----
mvn clean install
----
. *Run the relevant main classes*:
+
